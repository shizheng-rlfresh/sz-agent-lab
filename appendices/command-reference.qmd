...
Run these commands in this order when preparing a release or publishing rendered versions of the book. `pytest` should fail fast on broken runtime behavior. `python scripts/run_all_examples.py` should then prove that deterministic examples still produce trace, eval, and report artifacts. `make html` and `make pdf` should run last because manuscript rendering depends on the files and references that the implementation describes.
...
`make pdf` runs `quarto render --to pdf --no-clean` and produces `_book/Agentic-Systems-Lab.pdf`. PDF rendering uses Quarto's native PDF/LaTeX pipeline and requires a working PDF toolchain such as TinyTeX.

`make book` renders both HTML and PDF.

`make check` runs tests, deterministic examples, and both book renders.
...
`make html` proves the HTML book builds. `make pdf` proves the PDF book builds through Quarto's PDF toolchain. Neither command proves every claim is sufficiently sourced.
...
- If `make html` or `make pdf` fails, inspect the first `.qmd` file named in the render log. Common failures are malformed fenced code blocks, broken citation keys, invalid Quarto structure, or missing PDF toolchain dependencies.
...
| `make pdf` fails on toolchain | PDF dependency | inspect Quarto/TinyTeX availability and missing LaTeX package messages |
...
For manuscript-only changes, the minimum check is `make html`. If the prose affects PDF-sensitive structure, references, or generated book artifacts, also run `make pdf`. If the prose references commands, schemas, sample output, or generated reports, also run the corresponding command or test. A chapter can be editorial, but command claims are still executable claims.
